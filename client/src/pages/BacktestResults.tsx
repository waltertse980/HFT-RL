...
import { Play, Download, ChevronLeft, ChevronRight, TrendingUp, AlertTriangle, Loader2 } from "lucide-react";
      ...
      <div className="p-2 space-y-1">
        {isLoading &&
          Array.from({ length: 3 }).map((_, i) => (
            <Skeleton key={i} className="h-16 rounded-md bg-muted" />
          ))}
        {!isLoading && runs.length === 0 && (
          <p className="text-xs text-muted-foreground font-sans px-2 py-4 text-center">No past runs yet</p>
        )}
        {runs.map((run) => (
          <button
            key={run.id}
...
export default function BacktestResults() {
  ...
  const [selectingPastId, setSelectingPastId] = useState<number | null>(null);

  const tickers = market === "US" ? US_TICKERS : HK_TICKERS;

  // Fetch models from engine
  const { data: modelsData } = useQuery<{ models: string[] }>({
    queryKey: ["/api/models"],
    retry: false,
  });
  const STATIC_MODELS = ["PPO_US_1m_v3", "TD3_HK_5m_v2", "PPO_US_10s_v1", "PPO_HK_1m_v2"];
  const modelList =
    modelsData?.models && modelsData.models.length > 0
      ? modelsData.models
      : STATIC_MODELS;
  const [model, setModel] = useState(modelList[0] ?? "PPO_US_1m_v3");

  // Fetch past runs from DB
  const { data: pastRunsData, isLoading: pastRunsLoading } = useQuery<{
    results: Omit<BacktestRun, "equityCurve" | "trades">[];
  }>({
    queryKey: ["/api/backtest/results"],
    retry: false,
    refetchInterval: 0,
  });
  const pastRuns = pastRunsData?.results ?? [];

  // Run backtest mutation — calls real Python engine
  const runMutation = useMutation({
    mutationFn: async () => {
      // Use raw fetch so we can inspect the body for ENGINE_OFFLINE before throwing
      const res = await fetch("/api/backtest/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ market: market.toLowerCase(), ticker, timescale, modelPath: model, startDate, endDate }),
      });
    ...
    onSuccess: (data) => {
      ...
      setEngineError(null);
      setPage(0);
      qc.invalidateQueries({ queryKey: ["/api/backtest/results"] });
    },
    onError: (err: Error) => {
      if (err.message === "ENGINE_OFFLINE") {
        ...
        setEngineError(`Backtest failed: ${err.message}`);
      }
    },
  });

  // Select past run — fetches from DB by id
  const handleSelectPast = async (id: number) => {
    setSelectingPastId(id);
    setResult(null);
    setEngineError(null);
    setPage(0);
    try {
      ...
      setResult(data as BacktestRun);
      setSelectedRunId(id);
    } catch (err: any) {
      setEngineError(`Failed to load past run: ${err.message}`);
    } finally {
      setSelectingPastId(null);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = result.trades.map(
      (t) =>
        `${t.id},${t.time},${t.action},${t.price.toFixed(2)},${t.qty},${t.pnl.toFixed(2)},${t.cumulativePnl.toFixed(2)}`
    );
  ...
  const isLoading = runMutation.isPending || selectingPastId !== null;
              ...
              <div className="flex flex-wrap items-end gap-3">
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      setMarket(v);
                      setTicker(v === "US" ? "AAPL" : "0700.HK");
                    }}
                    data-testid="bt-market-select"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <Select value={model} onValueChange={setModel} data-testid="bt-model-select">
                    <SelectTrigger className="h-8 w-40 text-xs font-mono bg-muted border-border">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-popover border-border">
                      {modelList.map((m) => (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3" data-testid="metrics-grid">
                <MetricCard
                  ...
                  value={result.sharpe}
                  target="Target: >1.5"
                  good={result.sharpe > 1.5}
                  bad={result.sharpe <= 1.0}
                  testId="metric-sharpe"
                />
                <MetricCard
                  label="Sortino Ratio"
                  value={result.sortino}
                  good={result.sortino > 1.5}
                  testId="metric-sortino"
                />
                <MetricCard
                  label="Calmar Ratio"
                  value={result.calmar}
                  good={result.calmar > 1.0}
                ...
                <MetricCard
                  ...
                  suffix="%"
                  bad={Math.abs(result.maxDrawdown) > 15}
                  good={Math.abs(result.maxDrawdown) <= 10}
                ...
                <Card className="bg-card border-border" data-testid="metric-win-rate">
                  <CardContent className="p-4">
                    ...
                    <p
                      className={cn(
                        "text-2xl font-mono font-semibold",
                        result.winRate > 55 ? "text-success" : "text-warning"
                      )}
                    >
                      {result.winRate.toFixed(1)}%
                    </p>
                    <p className="text-xs text-muted-foreground font-sans mt-1">
                      {result.nTrades} trades
                    </p>
                  </CardContent>
                </Card>
              ...
              <Card className="bg-card border-border" data-testid="trade-log-table">
                    ...
                    <thead className="border-b border-border">
                      <tr>
                        {["#", "Time", "Action", "Price", "Qty", "P&L", "Cumulative P&L"].map((h) => (
                          <th
                            key={h}
                            className="px-3 py-2 text-left text-muted-foreground font-sans font-medium"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {pageTrades.map((trade) => (
                        ...
                        >
                          ...
                          <td className={cn("px-3 py-1.5 font-mono font-semibold", actionClass(trade.action))}>
                            {trade.action}
                          </td>
                          ...
                          <td
                            className={cn(
                              "px-3 py-1.5 font-mono",
                              trade.cumulativePnl >= 0 ? "text-success" : "text-destructive"
                            )}
                          >
                ...
                {totalPages > 1 && (
                  <div
                    ...
                    data-testid="trade-log-pagination"
                  >
